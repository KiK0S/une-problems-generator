...
'''.format(a, b, c[0])
	...
	def f(output, a, b, c):
		...
		assert bst != c[2]
		return cur + bst
	return (template, f(output, a, b, c), strep(output, pairs=1))
...
'''.format(a, b)
	...
	def f(output, a, b):
		...
		assert bst != -(10 ** 10)
		return cur + bst
	return (template, f(output, a, b), strep(output))

print(json.dumps(choice([generate_subsets, generate_pairs])(choice(range(10000, 20000)))))
